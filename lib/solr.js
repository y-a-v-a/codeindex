...
function queryIndex(string, fn) {
    ...
    var request = client.search('q=content_t:' + encodeURIComponent(string) + '&start=0&rows=3&hl=true&hl.fl=content_t&hl.snippets=4&hl.mergeContiguous=true&hl.fragsize=200&hl.bs.maxScan=50&hl.bs.type=LINE&hl.useFastVectorHighlighter=true&hl.tag.pre=%20&hl.tag.post=%20', function(err, obj) {
        if (err) {
            console.log(err);
            fn(err);
        }
    ...
    if (obj.highlighting) {
        ...
        for (var i = 0; i < keys.length; i++) {
            obj.highlighting[keys[i]].content_t = obj.highlighting[keys[i]].content_t.map(function(el) {
                var r = prism.highlight(el, prism.languages.javascript);
                return r;
            });
        }
    }
